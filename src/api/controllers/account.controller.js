...
const { service_name, eosd, faucet ,chain } = require('../../config/vars');
...
/* eslint-disable camelcase */
exports.createAccountFromFaucet = async (req, res, next) => {
  try {
    ...
    if(resp.status === 201){//包装返回值
      json.address = accountName;
      json.chainName = chainName;
      json.keys = keys;
      json.chainName = chain.eos.code;
      response.data = json;
